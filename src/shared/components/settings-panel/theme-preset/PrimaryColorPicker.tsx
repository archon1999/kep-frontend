...
const PrimaryColorPicker = ({ variant = 'default' }: PrimaryColorPickerProps) => {
  ...
  const primarySwatches = useMemo(() => Array.from(new Set(COLOR_GROUPS.map((group) => group.main))), []);
  ...
  return (
    ...
    >
      <Typography variant="subtitle2" color="text.secondary" fontWeight={600} sx={{ minWidth: 100 }}>
        Primary Color
      </Typography>
      <Box
        sx={{
