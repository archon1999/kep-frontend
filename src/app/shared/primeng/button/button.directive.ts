import { AfterViewInit, booleanAttribute, Directive, ElementRef, Inject, Input, OnDestroy } from '@angular/core';
import { DOCUMENT } from '@angular/common';
import { DomHandler } from '@primeng/dom';
import { ObjectUtils } from '@primeng/utils';
import { ButtonIconPosition, INTERNAL_BUTTON_CLASSES } from './button.constants';
...
export class ButtonDirective implements AfterViewInit, OnDestroy {
  ...
  updateLabel() {
    const labelElement = DomHandler.findSingle(this.htmlElement, '.p-button-label');

    if (!this.label) {
      if (labelElement) {
        this.htmlElement.removeChild(labelElement);
      }
      return;
    }
  ...
  updateIcon() {
    const iconElement = DomHandler.findSingle(this.htmlElement, '.p-button-icon');
    const labelElement = DomHandler.findSingle(this.htmlElement, '.p-button-label');
