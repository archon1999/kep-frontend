                    ...
                    <ng-container *ngIf="!itemTemplate">
                        <a
                            *ngIf="_visible && isClickableRouterLink(item); else elseBlock"
                            ...
                            [ngClass]="{ 'p-disabled': getDisabledValue(item) }"
                            role="menuitem"
                            [routerLinkActiveOptions]="item.routerLinkActiveOptions || { exact: false }"
                            (click)="onItemClick($event, item)"
                            (keydown.enter)="onItemClick($event, item, i)"
                            [attr.target]="item.target"
                            [attr.tabindex]="getDisabledValue(item) || readonly || !visible ? null : item.tabindex ? item.tabindex : '0'"
                        ...
                        <ng-template #elseBlock>
                            <a
                                *ngIf="_visible"
                                ...
                                role="menuitem"
                                pRipple
                                (click)="!getDisabledValue(item) && onItemClick($event, item)"
                                [ngClass]="{ 'p-disabled': getDisabledValue(item) }"
                                (keydown.enter)="onItemClick($event, item, i)"
                                [attr.target]="item.target"
                                [attr.data-pc-section]="'action'"
                                [attr.aria-label]="item.label"
                                [attr.tabindex]="getDisabledValue(item) || (i !== activeIndex && readonly) || !visible ? null : item.tabindex ? item.tabindex : '0'"
                            >
                                <span class="p-speeddial-action-icon" *ngIf="item.icon" [ngClass]="item.icon"></span>
                            </a>
                        </ng-template>
...
export class SpeedDial implements AfterViewInit, AfterContentInit, OnDestroy {
    ...
    getDisabledValue(item: MenuItem) {
    ...
    isClickableRouterLink(item: MenuItem) {
        return item.routerLink && !this.getDisabledValue(item);
    }
